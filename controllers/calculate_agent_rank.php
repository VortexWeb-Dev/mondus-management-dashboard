...
function calculateAgentRank()
{
    ...
    if (file_exists($cache_file)) {
        ...
        $deal_selects = ['BEGINDATE', 'ASSIGNED_BY_ID', 'UF_CRM_1741000938260'];
        $deal_orders = ['UF_CRM_1741000938260' => 'DESC', 'BEGINDATE' => 'DESC'];
        ...
        function store_agents($sorted_deals, &$global_ranking)
        {
            foreach ($sorted_deals as $deal) {
                ...
                if (true) { // Adjust condition if necessary
                    ...
                    $gross_comms = isset($deal['UF_CRM_1741000938260']) ? (int)explode('|', $deal['UF_CRM_1741000938260'])[0] : 0;

                    $agent = getUser($responsible_agent_id);
                    $agent_full_name = ($agent['NAME'] ?? '') . ' ' . ($agent['LAST_NAME'] ?? '');

                    $global_ranking[$year]['weekwise_rank'][$week][$responsible_agent_id]['name'] = $agent_full_name;
                    $global_ranking[$year]['weekwise_rank'][$week][$responsible_agent_id]['gross_comms'] =
                        ($global_ranking[$year]['weekwise_rank'][$week][$responsible_agent_id]['gross_comms'] ?? 0) + $gross_comms;
        ...
        assign_rank($global_ranking, 'weekwise_rank');
